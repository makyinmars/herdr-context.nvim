...
test("submission uses the agent-aware prompt command with exact payload bytes", function()
  local log = vim.fn.tempname()
  vim.fn.writefile({}, log)
  vim.env.FAKE_HERDR_LOG = log
  local cfg = config.setup({
  ...
  local ok, err, result = stage_and_wait(cfg, { pane_id = "w1:p2", agent = "codex" }, "line one\nline two")
  truthy(ok, err)
  eq({ mode = "agent_prompt", submitted = true }, result)
  local output = read_log(log)
  contains(output, "agent prompt")
  contains(output, "prompt=6c696e65206f6e650a6c696e652074776f")
  truthy(not output:find("pane send%-text"), "explicit submission must not stage separately")
  truthy(not output:find("pane send%-keys"), "explicit submission must not press Enter separately")
  vim.fn.delete(log)
end)
